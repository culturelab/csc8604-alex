...
namespace AlexTheAdventurous
{
    ...
    {
        private const string ServerUri = "http://alex-pi/web";
        ...
        private DispatcherTimer _downloadTasks;
        ...
        public AlexDataSource()
        {
            ...
            _downloadTasks = new DispatcherTimer();
            _downloadTasks.Interval = TimeSpan.FromSeconds(DownloadTasksIntervalSeconds);
            _downloadTasks.Tick += DownloadTasks;
        }
        ...
        public ReadOnlyObservableCollection<AlexTask> Tasks
        {
            get
            {
                ...
                {
                    // first time the list of tasks was requested
                    // start downloading them and return the collection that will receive them
                    _tasksDownloadStarted = true;
                    DownloadTasks(null, null);
                    _downloadTasks.Start();
                }                
        ...
        private async void DownloadTasks(object sender, object arg)
        {
            // tasks are returned as simple text response, one task per line
